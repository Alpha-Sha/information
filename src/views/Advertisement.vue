    ...
    <div class="list">
      <div class="topInfo" v-for="(item, index) in getMarketList" :key="index">
        ...
        <div>
          ...
          <el-button type="danger" @click="handleCloseAdvertisement(item)" v-if="(item.status == 1 || item.status == 2) && (item.publisher == account || owner)">关闭信息 </el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      inputAddress: '',
      createAdvertisementVisible: false, //发布信息弹窗
      marketList: [],
      menuList: [
        {
          name: '上架',
